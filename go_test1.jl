...
function go()
    ...
    numerical_parameters = Rayons.NumericalParameters(
        ...
        nmin=100, # cartesian grid size (for min time accumulation)
        maxdepth=200, # max trajectory tree depth : probably unused (ray stop because of power decay or max time reached)
        nα=200000, # number of initial angles for rays 
        tmax=2.e-2, # max time allowed for rays
        power_threshold=1.e-4, #min power allowed for rays
